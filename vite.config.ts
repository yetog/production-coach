...
export default defineConfig({
  ...
  server: {
    proxy: {
      // The browser requests `${BASE}api/...` because that is what
      // resolveApiBase() produces. Vite matches proxy keys as a prefix of the
      // request path, so the key has to include the base - the old '/api'
      // rule never matched and every dev request fell through to the SPA.
      // The server itself mounts routes at /api, hence the rewrite.
      [`${BASE}${API_PATH_SUFFIX}`]: {
        target: API_SERVER,
        changeOrigin: true,
        rewrite: (path: string) => path.replace(BASE.replace(/\/$/, ''), ''),
      },
    },
  },
})
